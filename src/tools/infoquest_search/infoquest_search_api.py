...
class InfoQuestAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        logger.info("Initializing InfoQuest Search API client")
        ...
        logger.info("InfoQuest environment validation successful")
        return values
    ...
    async def raw_results_async(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            query_truncated = query[:50] + "..." if len(query) > 50 else query
            logger.debug(
                f"InfoQuest Search API async request initiated | "
                f"operation=search | "
                f"query_truncated={query_truncated} | "
                f"has_time_filter={time_range > 0} | "
                f"has_site_filter={bool(site)} | "
