...
Then restart the TaskPilot server.
...
When using MCP tools in TaskPilot, you need to specify:

1. **`enabled_tools`** - Which tools from the MCP server to enable
2. **`add_to_agents`** - Which TaskPilot agents can use these tools (`researcher`, `coder`, or both)
...
TaskPilot provides configurable timeout settings for MCP server connections to handle various network conditions and server responsiveness scenarios.
