...
This is the web UI for TaskPilot.
...
- `NEXT_PUBLIC_API_URL`: The URL of the task-pilot API.
...
TaskPilot Web UI uses `pnpm` as its package manager.
...
```bash
docker build --build-arg NEXT_PUBLIC_API_URL=YOUR_TASK_PILOT_API -t task-pilot-web .
```

Final, start up a docker container running the web server:

```bash
# Replace task-pilot-web-app with your preferred container name
docker run -d -t -p 3000:3000 --env-file .env --name task-pilot-web-app task-pilot-web

# stop the server
docker stop task-pilot-web-app
```
...
TaskPilot is built upon the foundation of these outstanding projects:
...
These outstanding projects form the backbone of TaskPilot and exemplify the transformative power of open source collaboration.
